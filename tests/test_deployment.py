...
def _patch_cuda(monkeypatch, free_gb_by_index: list[float]) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda index: SimpleNamespace(total_memory=int(12 * 1024**3)),
    )
    monkeypatch.setattr(
        deployment.torch.cuda,
        "mem_get_info",
        lambda index: (int(free_gb_by_index[index] * 1024**3), int(12 * 1024**3)),
    )
